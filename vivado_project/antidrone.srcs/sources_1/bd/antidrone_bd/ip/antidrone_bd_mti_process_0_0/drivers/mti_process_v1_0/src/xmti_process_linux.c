...
typedef struct {
    ...
    XMti_process_uio_map maps[ MAX_UIO_MAPS ];
} XMti_process_uio_info;

/***************** Variable Definitions **************************************/
static XMti_process_uio_info uio_info;
...
static int uio_info_read_name(XMti_process_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMti_process_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMti_process_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMti_process_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMti_process_Initialize(XMti_process *InstancePtr, const char* InstanceName) {
	XMti_process_uio_info *InfoPtr = &uio_info;
