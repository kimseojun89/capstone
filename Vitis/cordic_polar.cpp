...
#pragma HLS INTERFACE s_axilite port=y_in
#pragma HLS INTERFACE s_axilite port=distance
#pragma HLS INTERFACE s_axilite port=angle_deg
...
#pragma HLS BIND_STORAGE variable=ATAN_TABLE_TENTHS type=ROM_1P impl=LUTRAM
    ...
    int angle_offset = 0;
    if (x < 0) {
        /* Rotate by +180 or -180 depending on y sign */
        if (y >= 0) {
            angle_offset = 1800;  /* +180 deg in 0.1-deg */
        } else {
            angle_offset = -1800;
        }
        x = -x;
